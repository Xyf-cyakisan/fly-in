...
class MapConfig:
    ...
    def _check_mandatory_data(line: str, line_number: int) -> list[str]:
        ...
        if "hub" in line[0] and len(list_data) != 3:
            raise ValueError(
                ...
                "must have 3 values (<name> <x> <y>)"
            )
        elif "connection" in line[0]:
            if len(list_data) != 1:
                raise ValueError(
                    f"Error (line {line_number}): {line[0]} "
                    "must have 1 value (<zone_name1-zone_name2>)"
                )
            ...
            raise ValueError(
                f"Error (line {line_number}): {line[0]} "
                "must have 1 value (<non_zero_integer>)"
            )
        return list_data
    ...
    def _check_hub_names(dict_content) -> None:
        ...
        if "-" in dict_content["start_hub"][0]:
            raise ValueError(
                "Error (line "
                f"{dict_content['lines'][dict_content['start_hub'][0]]}): name"
                f" '{dict_content['start_hub'][0]}' should not have the "
                "'-' character in them"
            )
        ...
        if dict_content["end_hub"][0] in names:
            raise ValueError(
                ...
                f"name '{dict_content['end_hub'][0]}' already exists"
            )
        elif "-" in dict_content["end_hub"][0]:
            raise ValueError(
                "Error (line "
                f"{dict_content['lines'][dict_content['end_hub'][0]]})"
                f": name '{dict_content['end_hub'][0]}' should not have the "
                "'-' character in them"
            )
        ...
        for name, _, _ in dict_content["hub"]:
            if name in names:
                raise ValueError(
                    f"Error (line {dict_content['lines'][name]}): name "
                    f"'{name}' already exists"
                )
            elif "-" in name:
                raise ValueError(
                    f"Error (line {dict_content['lines'][name]}): name "
                    f"'{name}' should not have the '-' character in them"
                )
            names.append(name)
        for connection in dict_content["connection"]:
            if connection[0] not in names or connection[1] not in names:
                not_present = (
                    connection[0]
                    if connection[0] not in names
                    else connection[1] if connection[1] not in names else ""
                )
                raise ValueError(
                    "Error (line " +
                    dict_content['lines'][connection[0] + '-' + connection[1]]
                    + f"): '{not_present}' does not exists"
                )
        ...
        for i, connection_to_check in enumerate(dict_content["connection"]):
            for y, connection in enumerate(dict_content["connection"]):
                if i != y and set(connection) == set(connection_to_check):
                    raise ValueError(
                        "Error (line " +
                        dict_content['lines'][connection[0] +
                                              '-' + connection[1]] +
    ...
    def _all_data_types_covered(cls, dict_content) -> None:
        ...
        if not_passed != []:
            raise ValueError(
                f"Error: these value_type are missing "
    ...
    def _convert_hubs_value_type(dict_content) -> dict:
        ...
        try:
            ...
            raise ValueError(
                "Error (line "
                f"{dict_content['lines'][dict_content['start_hub'][0]]}): "
                "coordinates x and y need to be integers"
            )
        try:
        ...
        try:
            ...
            raise ValueError(
                "Error (line "
                f"{dict_content['lines'][dict_content['hub'][i][0]]}): "
                "coordinates x and y need to be integers"
            )
        return dict_content
        ...
        for i, line in zip(lines, content):
            if line.count(":") == 0:
                ...
                if len(line) != 2 or line[0] not in cls.POSSIBLE_VALUES:
                    ...
                    if (
                        len(dict_content["start_hub"]) > 1
                        or len(dict_content["end_hub"]) > 1
                    ):
                        raise ValueError(
                            f"Error (line {i}): Only 1 start_hub and end_hub"
                        )
        return dict_content
    ...
    def _check_metadata_type(dict_content) -> None:
        ...
        for hub in dict_content["hub"]:
            ...
            ) and dict_content["metadata"][hub[0]].get("max_drones", None):
                try:
                    dict_content["metadata"][hub[0]]["max_drones"] = int(
    ...
    def _check_coordinates_duplicate(dict_content) -> "MapConfig":
        ...
        for hub_to_check in dict_content["hub"]:
            if (
                hub_to_check[1] == dict_content["start_hub"][1]
                ...
                hub_to_check[1] == dict_content["end_hub"][1]
            ...
            for hub in dict_content["hub"]:
                if (
                    hub_to_check[1] == hub[1]
                    and hub_to_check[2] == hub[2]
...
if __name__ == "__main__":
    try:
        ...
        print(
            ...
            parsed.start_hub,
            "metadata: ",
            parsed.metadata[parsed.start_hub[0]],
        )
        print(
            "end: ",
            parsed.end_hub,
            "metadata: ",
            parsed.metadata[parsed.end_hub[0]],
        )
        print(
            "drones: ",
            parsed.nb_drones,
            "metadata: ",
            parsed.metadata["nb_drones"],
        )
        print()
        ...
        for connection in parsed.connection:
            print(
                ...
                connection[1],
                "metadata: ",
                parsed.metadata[connection[0] + "-" + connection[1]],
            )
